...
mod tests {
    ...
    async fn test_groth16_proof() {
        ...
        let mut storage_process = db.acquire().await.unwrap();
        let groth16_vk_v = storage_process.get_groth16_vk(ZKM_CIRCUIT_VERSION).await.unwrap();

        assert_eq!(groth16_vk, latest_groth16_vk);
        assert_eq!(groth16_vk, groth16_vk_v);
